...
path!(
    ...
    /// process, and the `.` and `..` components are removed, so that the
    /// walk never passes through a directory that the developer did not
    /// name. Symbolic links are not resolved, so the walk sees the tree that
    /// the developer named.
    ///
    /// The value can name a file. The walk then starts at the directory that
    /// holds the file, because the project that governs a file is the
    /// project of that directory.
    ///

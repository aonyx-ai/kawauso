...
2. **Requirements have identifiers.** A specification is a list of
   ...
   Each requirement has an identifier such as `kawauso[placeholder.add]`. The
   prefix is the name of the crate without `kawauso-`.

3. **The code has references to the requirements.** A comment such as
   `// kawauso[impl placeholder.add]` marks the code that implements a
   requirement. A comment such as `// kawauso[verify placeholder.add]` marks
...
4. **Requirements have versions.** When the text of a requirement changes,
   its version increases, for example to `kawauso[placeholder.add+2]`. A
...
are independent, and applications can use each crate on its own. The
